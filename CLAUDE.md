...
3. `ranker` — normalizes brand/model names, deduplicates, computes Heat Scores, resolves rank changes
4. `geniuslink_client` — creates or retrieves cached `geni.us` affiliate links per ASIN
5. `content_generator` — calls Anthropic API with structured product data, validates JSON output
6. `airtable_client` — upserts to three shared Airtable tables, partitioned by `category_id`
...
- **Affiliate links:** GeniusLink API (`POST /v3/links`)
...
```
models.py defines → RankedProduct, WeeklyRoundup, CatalogEntry, RunLog, CategoryConfig
...
Every `intro_html`, `why_hot_html`, `short_specs_html`, and FAQ block written to Airtable must be fit for this reader. This is an editorial quality standard, not just a formatting requirement — if the content reads like a spec sheet, it has failed.
...
- Briefly explain the ranking method in plain language (sales momentum + ratings — no formula details)
...
- Expand factual product data (BSR, rating, review count, price, category tags) into readable HTML in Active Amy's voice
...
- Format data into `<ul>`, `<details>/<summary>`, and `<p>` HTML structures
...
- Fabricate sources, URLs, review quotes, or endorsements
...
The Heat Score formula is in `ranker.py`. It is explicitly **not** in config because changes to the formula change the meaning of historical data — they should be deliberate code changes, not config tweaks.
