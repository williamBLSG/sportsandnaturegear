"""Config loader — reads softball article YAML and injects runtime secrets."""
...
from softball_pipeline.models import SoftballArticleConfig

logger = logging.getLogger(__name__)

_CONFIG_DIR = Path(__file__).resolve().parent.parent.parent / "config" / "softball"
...
def load(article_id: str) -> SoftballArticleConfig:
    """Load and validate a softball article config from YAML.
    ...
    logger.info("Loading softball config: %s", config_path)
    ...
    try:
        config = SoftballArticleConfig(**resolved_data)
    except ValidationError as e:
        raise ConfigLoaderError(
            f"Config validation failed for '{article_id}': {e}"
        ) from e

    logger.info("Softball config loaded: %s (%s)", config.display_name, config.article_id)
    return config


def list_article_ids() -> list[str]:
    """Return all article IDs from YAML files in the softball config directory."""
