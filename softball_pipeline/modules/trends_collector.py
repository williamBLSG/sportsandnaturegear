"""Trends collector — fetches Google Trends rising + top queries for softball articles.

Reuses the same pytrends + Claude classification pattern as the weekly trending
pipeline but adapted for softball-specific config (SoftballArticleConfig,
softball brand lists, per-article trends_keyword).
"""
...
def _fetch_trends(config: SoftballArticleConfig) -> tuple[dict, str]:
    ...
    try:
        pytrends.build_payload(
            [keyword],
            timeframe="today 3-m",  # Last 3 months — captures seasonal softball interest
            geo="US",
        )
...
def _classify_queries(
...
) -> tuple[list[SoftballTrendsQuery], list[SoftballTrendsQuery]]:
    ...
    # Use softball-specific brands from config, supplemented with common softball brands
    ...
    prompt = f"""Classify each Google Trends query for the "{config.display_name}" category (softball gear).

Known softball brands: {', '.join(sorted(known_brands))}
...
- query_type: "brand_only" if it mentions a brand but no specific model (e.g., "rawlings softball gloves", "easton fastpitch bats")
- query_type: "generic" if no brand is mentioned (e.g., "best softball gloves", "fastpitch bat reviews")
...
def collect(
    ...
    """Collect Google Trends data for a softball article.
    ...
    """
    artifact_path = softball_runs_path(config.article_id, run_date, "trends.json")

    if artifact_path.exists() and not force:
        logger.info("Resuming from cached trends.json")
        return SoftballTrendsData.model_validate_json(artifact_path.read_text())
