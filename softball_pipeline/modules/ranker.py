"""Ranker — normalizes, deduplicates, scores, and ranks softball products.
...
from softball_pipeline.models import (
    ...
    SoftballRankedOutput,
    SoftballRankedProduct,
    SoftballRawProduct,
    SoftballRawSignals,
    SoftballTrendsData,
    SoftballTrendsQuery,
...
def _normalize_brands(
    ...
    prompt = f"""Extract the brand name and model name from each softball product title.

Known softball brands: {', '.join(known_brands)}

Rules:
- Brand should be title-case and match the known brand list when possible
- Model should be the specific product name WITHOUT color, size, gender, or "softball" qualifier
  Example: "Rawlings Liberty Advanced Color Series 13" Fastpitch Softball Glove - RLA130-6RS" → brand: "Rawlings", model: "Liberty Advanced"
  Example: "Louisville Slugger LXT -10 Fastpitch Softball Bat" → brand: "Louisville Slugger", model: "LXT"
  Example: "Dudley Thunder ZN 12\" ASA Slowpitch Softball" → brand: "Dudley", model: "Thunder ZN"
...
def _deduplicate(
...
) -> list[tuple[SoftballRawProduct, dict]]:
    ...
    groups: dict[tuple[str, str], list[tuple[SoftballRawProduct, dict]]] = {}
    for p in products:
...
def _compute_brand_authority_score(brand: str, config: SoftballArticleConfig) -> float:
...
def _compute_trends_score(
    brand: str,
    model: str,
    trends: SoftballTrendsData | None,
...
def _assign_roles(products: list[SoftballRankedProduct]) -> None:
...
def rank(
    ...
    config: SoftballArticleConfig,
    run_date: str,
    trends: SoftballTrendsData | None = None,
    force: bool = False,
) -> SoftballRankedOutput:
    """Normalize, deduplicate, score, and rank softball products.

    Idempotent: skips work if ranked.json exists (unless force=True).
    Returns SoftballRankedOutput with products sorted by composite score.
    """
    artifact_path = softball_runs_path(config.article_id, run_date, "ranked.json")

    if artifact_path.exists() and not force:
        logger.info("Resuming from cached ranked.json")
        return SoftballRankedOutput.model_validate_json(artifact_path.read_text())
    ...
    scored_products: list[SoftballRankedProduct] = []
    for product, norm in deduped:
        ...
        scored_products.append(SoftballRankedProduct(
