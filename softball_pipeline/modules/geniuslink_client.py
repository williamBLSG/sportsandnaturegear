...
with softball-specific config (global per-category cache in runs/softball/).
...
# ---------------------------------------------------------------------------
# Global ASIN cache (shared across all softball articles)
# ---------------------------------------------------------------------------

def _load_cache() -> dict[str, str]:
    """Load the global softball ASIN → geni.us URL cache."""
    cache_file = softball_cache_path("geniuslink_cache.json")
...
def _save_cache(cache: dict[str, str]) -> None:
    """Save the global softball ASIN → geni.us URL cache."""
    cache_file = softball_cache_path("geniuslink_cache.json")
...
def _set_utm_tags(
    ...
    """Set UTM post-processing rules for the softball group.
...
def enrich(
    ranked: SoftballRankedOutput,
    config: SoftballArticleConfig,
    run_date: str,
    force: bool = False,
) -> list[SoftballLinkedProduct]:
    """Create or retrieve GeniusLink affiliate URLs for each ranked product.

    Uses a global per-category ASIN cache (runs/softball/geniuslink_cache.json)
    to avoid creating duplicate links.

    Idempotent: skips work if linked.json exists (unless force=True).
    Returns list of SoftballLinkedProduct with affiliate_url populated.
    """
    artifact_path = softball_runs_path(config.article_id, run_date, "linked.json")

    if artifact_path.exists() and not force:
        logger.info("Resuming from cached linked.json")
        data = json.loads(artifact_path.read_text())
        return [SoftballLinkedProduct(**p) for p in data]
    ...
    linked_products: list[SoftballLinkedProduct] = []
    cached_count = 0
    created_count = 0
    failed_count = 0
