"""Pydantic schemas — canonical data contract for the softball article pipeline.

All data shapes are defined here. No module defines its own ad-hoc dicts.

Softball pipeline differs from the weekly trending pipeline:
...
def softball_runs_path(
    article_id: str,
    run_date: str,
    filename: str | None = None,
) -> Path:
    """Return the path to a softball run artifact directory or file.

    Creates the directory if it doesn't exist.
    Structure: runs/softball/{article_id}/{run_date}/
    """
    base = Path(__file__).resolve().parent.parent / "runs" / "softball" / article_id / run_date
...
def softball_cache_path(filename: str | None = None) -> Path:
    """Return the path to the global softball cache directory.

    Structure: runs/softball/
    Used for GeniusLink ASIN cache shared across all articles.
    """
    base = Path(__file__).resolve().parent.parent / "runs" / "softball"
    base.mkdir(parents=True, exist_ok=True)
    if filename:
        return base / filename
    return base
...
# ---------------------------------------------------------------------------
# Article config (loaded from YAML in config/softball/)
# ---------------------------------------------------------------------------
...
class SoftballArticleConfig(BaseModel):
    article_id: str
    category_id: str  # Always "softball"
    display_name: str
    site_name: str
    site_url: str
    ...
    table_articles: str  # "softball-articles"
    table_products: str  # "softball-products"
    table_faq: str  # "softball-faq"
...
class SoftballRawProduct(BaseModel):
    asin: str
    title: str
...
class SoftballRawSignals(BaseModel):
    article_id: str
    category_id: str
...
class SoftballTrendsQuery(BaseModel):
    query: str
...
class SoftballTrendsData(BaseModel):
    article_id: str
    collected_at: datetime
    trends_keyword: str
    rising_queries: list[SoftballTrendsQuery]
    top_queries: list[SoftballTrendsQuery]
...
class SoftballRankedProduct(BaseModel):
    rank: int
    asin: str
    title: str
    brand: str
...
class SoftballRankedOutput(BaseModel):
    article_id: str
    category_id: str
    ranked_at: datetime
    product_count: int
    products: list[SoftballRankedProduct]
...
class SoftballLinkedProduct(BaseModel):
...
class SoftballProductContent(BaseModel):
...
class SoftballArticleContent(BaseModel):
    ...
    # Widget 4: "The Three Tiers" — budget / mid-range / premium side-by-side cards (inline CSS)
    widget_4: str  # HTML blob with inline CSS

    # Widget 5: "Top Features to Look For" section
    widget_5: str  # HTML blob

    # Widget 6: Comparison table — "Our Top Picks at a Glance" (inline CSS)
    ...
    products: list[SoftballProductContent]

    # FAQ entries for Airtable FAQ table (plain text only)
    faqs: list[SoftballFaqEntry]
...
class SoftballRunLog(BaseModel):
    article_id: str
    category_id: str
    run_date: str
