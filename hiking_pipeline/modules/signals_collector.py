"""Signals collector — queries Amazon Creators API for hiking products.

Reuses the same Creators API pattern as the weekly trending pipeline but
with hiking-specific config (SportingGoods search index, no browse node,
no gender filter).
"""
...
def _get_api_client(config: HikingArticleConfig):
...
def _extract_product(item) -> HikingRawProduct | None:
    """Extract a HikingRawProduct from an Amazon Creators API result item.
...
def _fetch_products(
    ...
    No browse_node_id or gender filter for hiking.
    """
    ...
    all_products: list[HikingRawProduct] = []
...
def _apply_filters(
    ...
    if not filtered:
        # Log summary at WARNING level when everything gets filtered
        logger.warning(
...
def collect(
    ...
    Returns HikingRawSignals with filtered products.
    """
    artifact_path = hiking_runs_path(config.article_id, run_date, "raw_signals.json")

    if artifact_path.exists() and not force:
        logger.info("Resuming from cached raw_signals.json")
        return HikingRawSignals.model_validate_json(artifact_path.read_text())
