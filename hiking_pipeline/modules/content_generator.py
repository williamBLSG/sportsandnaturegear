...
Generates a complete HikingArticleContent with:
...
def _build_product_data(products: list[HikingLinkedProduct]) -> list[dict]:
...
def _build_prompt(
    products: list[HikingLinkedProduct],
...
"widget_1": "<p>Whether your daughter just signed up for her first rec league...</p><p>This article covers the best hiking boots available right now...</p>"
...
def _validate_faq_plain_text(content: HikingArticleContent) -> None:
...
def generate(
    ...
    """
    artifact_path = hiking_runs_path(config.article_id, run_date, "content.json")

    if artifact_path.exists() and not force:
        logger.info("Resuming from cached content.json")
        return HikingArticleContent.model_validate_json(artifact_path.read_text())
    ...
    try:
        ...
        content = HikingArticleContent(**data)
...
def regenerate_widget_for_price_change(
    product: HikingLinkedProduct,
    widget_number: int,
    config: HikingArticleConfig,
    old_price: float,
    new_price: float,
    all_tier_products: list[HikingLinkedProduct] | None = None,
) -> str:
