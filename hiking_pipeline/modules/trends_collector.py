"""Trends collector — fetches Google Trends rising + top queries for hiking articles.

Reuses the same pytrends + Claude classification pattern as the weekly trending
pipeline but adapted for hiking-specific config (HikingArticleConfig,
hiking brand lists, per-article trends_keyword).
"""
...
def _fetch_trends(config: HikingArticleConfig) -> tuple[dict, str]:
    ...
    try:
        pytrends.build_payload(
            [keyword],
            timeframe="today 3-m",  # Last 3 months — captures seasonal hiking interest
            geo="US",
        )
...
def _classify_queries(
...
) -> tuple[list[HikingTrendsQuery], list[HikingTrendsQuery]]:
    ...
    # Use hiking-specific brands from config, supplemented with common hiking brands
    ...
    prompt = f"""Classify each Google Trends query for the "{config.display_name}" category (hiking gear).

Known hiking brands: {', '.join(sorted(known_brands))}
...
- query_type: "brand_only" if it mentions a brand but no specific model (e.g., "merrell hiking boots", "salomon trail shoes")
- query_type: "generic" if no brand is mentioned (e.g., "best hiking boots", "hiking pole reviews")
...
def collect(
    ...
    """Collect Google Trends data for a hiking article.
    ...
    """
    artifact_path = hiking_runs_path(config.article_id, run_date, "trends.json")

    if artifact_path.exists() and not force:
        logger.info("Resuming from cached trends.json")
        return HikingTrendsData.model_validate_json(artifact_path.read_text())
