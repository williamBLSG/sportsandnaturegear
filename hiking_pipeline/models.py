"""Pydantic schemas — canonical data contract for the hiking article pipeline.

All data shapes are defined here. No module defines its own ad-hoc dicts.

Hiking pipeline differs from the weekly trending pipeline:
...
def hiking_runs_path(
    article_id: str,
    run_date: str,
    filename: str | None = None,
) -> Path:
    """Return the path to a hiking run artifact directory or file.

    Creates the directory if it doesn't exist.
    Structure: runs/hiking/{article_id}/{run_date}/
    """
    base = Path(__file__).resolve().parent.parent / "runs" / "hiking" / article_id / run_date
...
def hiking_cache_path(filename: str | None = None) -> Path:
    """Return the path to the global hiking cache directory.

    Structure: runs/hiking/
    Used for GeniusLink ASIN cache shared across all articles.
    """
    base = Path(__file__).resolve().parent.parent / "runs" / "hiking"
    base.mkdir(parents=True, exist_ok=True)
    if filename:
        return base / filename
    return base
...
# ---------------------------------------------------------------------------
# Article config (loaded from YAML in config/hiking/)
# ---------------------------------------------------------------------------
...
class HikingArticleConfig(BaseModel):
    article_id: str
    category_id: str  # Always "hiking"
    display_name: str
    site_name: str
    site_url: str
    ...
    table_articles: str  # "hiking-articles"
    table_products: str  # "hiking-products"
    table_faq: str  # "hiking-faq"
...
class HikingRawProduct(BaseModel):
    asin: str
    title: str
...
class HikingRawSignals(BaseModel):
    article_id: str
    category_id: str
...
class HikingTrendsQuery(BaseModel):
    query: str
...
class HikingTrendsData(BaseModel):
    article_id: str
    collected_at: datetime
    trends_keyword: str
    rising_queries: list[HikingTrendsQuery]
    top_queries: list[HikingTrendsQuery]
...
class HikingRankedProduct(BaseModel):
    rank: int
    asin: str
    title: str
    brand: str
...
class HikingRankedOutput(BaseModel):
    article_id: str
    category_id: str
    ranked_at: datetime
    product_count: int
...
class HikingLinkedProduct(BaseModel):
...
class HikingProductContent(BaseModel):
...
class HikingFaqEntry(BaseModel):
...
class HikingArticleContent(BaseModel):
    ...
    products: list[HikingProductContent]

    # FAQ entries for Airtable FAQ table (plain text only)
    faqs: list[HikingFaqEntry]
