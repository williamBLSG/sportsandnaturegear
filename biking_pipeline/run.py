...
Usage:
  python -m biking_pipeline.run --article fastpitch-biking-bats --type daily_build
  python -m biking_pipeline.run --article fastpitch-biking-bats --type price_check
  python -m biking_pipeline.run --article fastpitch-biking-bats --type manual_refresh
  python -m biking_pipeline.run --type daily_build  # Auto-picks next queued article
"""
...
from biking_pipeline.models import BikingRunLog, biking_runs_path

from biking_pipeline.modules import (
    airtable_client,
    config_loader,
    content_generator,
    geniuslink_client,
...
from biking_pipeline.modules.airtable_client import AirtableClientError
from biking_pipeline.modules.config_loader import ConfigLoaderError
from biking_pipeline.modules.content_generator import ContentGeneratorError
from biking_pipeline.modules.geniuslink_client import GeniusLinkError
from biking_pipeline.modules.ranker import RankerError
from biking_pipeline.modules.signals_collector import SignalsCollectorError
from biking_pipeline.modules.trends_collector import TrendsCollectorError
...
def _save_run_log(run_log: BikingRunLog, run_date: str) -> None:
    path = biking_runs_path(run_log.article_id, run_date, "run_log.json")
    ...
    try:
        ...
        for product in current_products:
            ...
            if pct_change > 10:
                ...
                if role == "top_pick":
                    try:
                        from biking_pipeline.models import BikingLinkedProduct
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Biking pipeline orchestrator")
    parser.add_argument(
        "--article",
        type=str,
        default=None,
        help="Article ID (e.g., fastpitch-biking-bats). If omitted with daily_build, "
