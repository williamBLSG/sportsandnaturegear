"""Config loader — reads biking article YAML and injects runtime secrets."""
...
from biking_pipeline.models import BikingArticleConfig

logger = logging.getLogger(__name__)

_CONFIG_DIR = Path(__file__).resolve().parent.parent.parent / "config" / "biking"
...
def load(article_id: str) -> BikingArticleConfig:
    """Load and validate a biking article config from YAML.
    ...
    logger.info("Loading biking config: %s", config_path)
    ...
    try:
        config = BikingArticleConfig(**resolved_data)
    except ValidationError as e:
        raise ConfigLoaderError(
            f"Config validation failed for '{article_id}': {e}"
        ) from e

    logger.info("Biking config loaded: %s (%s)", config.display_name, config.article_id)
    return config


def list_article_ids() -> list[str]:
    """Return all article IDs from YAML files in the biking config directory."""
