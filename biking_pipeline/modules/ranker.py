"""Ranker — normalizes, deduplicates, scores, and ranks biking products.
...
def _normalize_brands(
    ...
    prompt = f"""Extract the brand name and model name from each biking product title.

Known biking brands: {', '.join(known_brands)}

Rules:
- Brand should be title-case and match the known brand list when possible
- Model should be the specific product name WITHOUT color, size, gender, or "biking" qualifier
  Example: "Rawlings Liberty Advanced Color Series 13" Fastpitch Biking Glove - RLA130-6RS" → brand: "Rawlings", model: "Liberty Advanced"
  Example: "Louisville Slugger LXT -10 Fastpitch Biking Bat" → brand: "Louisville Slugger", model: "LXT"
  Example: "Dudley Thunder ZN 12\" ASA Slowpitch Biking" → brand: "Dudley", model: "Thunder ZN"
...
def _deduplicate(
...
) -> list[tuple[BikingRawProduct, dict]]:
    ...
    groups: dict[tuple[str, str], list[tuple[BikingRawProduct, dict]]] = {}
    for p in products:
...
def _compute_brand_authority_score(brand: str, config: BikingArticleConfig) -> float:
...
def _assign_roles(products: list[BikingRankedProduct]) -> None:
...
def rank(
    ...
    config: BikingArticleConfig,
    run_date: str,
    trends: BikingTrendsData | None = None,
    force: bool = False,
) -> BikingRankedOutput:
    """Normalize, deduplicate, score, and rank biking products.

    Idempotent: skips work if ranked.json exists (unless force=True).
    Returns BikingRankedOutput with products sorted by composite score.
    """
    artifact_path = biking_runs_path(config.article_id, run_date, "ranked.json")

    if artifact_path.exists() and not force:
        logger.info("Resuming from cached ranked.json")
        return BikingRankedOutput.model_validate_json(artifact_path.read_text())
    ...
    scored_products: list[BikingRankedProduct] = []
    for product, norm in deduped:
        ...
        scored_products.append(BikingRankedProduct(
