"""Airtable client — upserts biking article content, products, and FAQs.

Writes to three tables in the biking Airtable base:
  1. biking-articles  — article content (10 HTML widgets, SEO, ASIN assignments)
  2. biking-products   — per-product data (editorial blurbs, affiliate links)
  3. biking-faq        — FAQ entries (plain text only, no HTML)
...
Airtable schema (field names from existing base):
  biking-articles (tbl4p48LV1YDJ7teu):
  ...
  biking-products (tbl1CmZmPHhLSF9Pk):
...
def _upsert_article(
    ...
    """Upsert the article row in biking-articles table.
...
def _upsert_products(
    ...
    """Upsert product rows in biking-products table.
...
def _upsert_faqs(
    ...
    """Upsert FAQ rows in biking-faq table.
...
def update_product_price(
    ...
    """Update a single product's price in biking-products table."""
    api = _get_api()
    table = api.table(config.airtable_base_id, config.table_products)
...
def update_article_widget(
    ...
    """Update a single widget HTML in biking-articles table."""
    api = _get_api()
    table = api.table(config.airtable_base_id, config.table_articles)
